...
def niche_specs() -> list[CommandSpec]:
    ...
    return [
        ...
        CommandSpec("scan", "Scan all eight E2 addresses through production identity/status validation.", group="niche-bus", expected_all=("Address 0:", "FOUND compatible EE871", "Found 1 device(s)"), timeout_s=45.0),
        CommandSpec("timing", "Characterize the fixed in-spec diagnostic timing set.", group="niche-bus", expected_all=("Timing Discovery", "identity + PEC validated", "timing(s) worked"), timeout_s=45.0),
        CommandSpec("libtest", "Exercise nine identity, status, and CO2 control-byte reads.", group="niche-bus", expected_all=("Library Command Test", "Passed: 9/9"), timeout_s=45.0),
