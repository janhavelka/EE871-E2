...
| P-05 | Part name write/readback | S3 | `partname`, `dirty`, record value, `partname <bench_text>`, `partname`, `dirty` | Write returns OK and readback matches; dirty diagnostics checked on failure. | PASS | COM20 same-value `EE871` write returned OK and read back `EE871`; the ad-hoc session was not used as an overall PASS. Detailed artifacts remain recoverable from Git commit `3687e49`. |
